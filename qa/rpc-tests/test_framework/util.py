...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "bluehost.conf"), 'w') as f:
        f.write("regtest=1\n")
        f.write("rpcuser=rt\n")
...
def initialize_chain(test_dir):
    ...
    if (not os.path.isdir(os.path.join("cache","node0"))
        ...
        # Create cache directories, run bluehostds:
        for i in range(4):
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in bluehost.conf
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, binary=None):
    """
    Start multiple bluehostds, return RPC connections to them
    """
