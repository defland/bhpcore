...
*Setup instructions for a Gitian build of Bluehost Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bluehost
...
Clone the git repositories for Bluehost Core and Gitian.
...
in the Bluehost Core repository under 'Fetch and build inputs' to install sources which require
...
To build Bluehost Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Bluehost Core repository.
