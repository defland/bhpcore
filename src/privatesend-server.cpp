...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Bluehost related functionality
...
/*
    ...
    allow endless transaction that would bloat Bluehost and make it unusable. To
...
{
    if(fLiteMode) return; // disable all Bluehost specific functionality
